//
//  main.swift
//  
//
//  Created by Jakub Olejník on 11/12/2019.
//

import ACKLocalizationCore
import Foundation

let authAPI = AuthAPIService()
let sheetAPI = SheetsAPIService()
let account = ServiceAccount(
    clientEmail: "localizetest@garage-6a683.iam.gserviceaccount.com",
    privateKey: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n")
var accessToken: AccessToken?

let group = DispatchGroup()

group.enter()
let d = authAPI.fetchAccessToken(serviceAccount: account)
    .handleEvents(receiveOutput: { accessToken = $0 })
    .flatMap { sheetAPI.fetchSpreadsheet("1QTiNcphjSJ96vbxIqV0RI46Vl9EHCCfbUWBJVqMgC0Y", accessToken: $0) }
    .flatMap { sheetAPI.fetchSheet(nil, from: $0, accessToken: accessToken!) }
    .sink(receiveCompletion: { completion in
        print(completion)
    }) { value in
        print(value)
}

group.wait()
